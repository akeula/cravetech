...
const app = (function() {
    ...
    // Generate a unique deterministic database key based on the host domain and pathname
    function getDatabaseKey() {
        let path = window.location.host + window.location.pathname;
        path = path.split('?')[0].split('#')[0]; // strip query parameters
        ...
        return "inverter_db_" + path.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase();
    }
