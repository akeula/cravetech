        ...
        fetch(url)
            ...
            .then(payload => {
                if (payload && payload.tickets) {
                    ...
                    // Cloud is empty. If we have local data, initialize the cloud database immediately.
                    if (tickets && (tickets.length > 0 || staff.length > 0)) {
                        console.log("Cloud database empty. Initializing online database with local records...");
                        syncOnline();
                    }
                }
            ...
            .catch(err => {
                ...
                // If the key is not found (404), initialize it with local data if available
                if (err.message && err.message.includes("404") && tickets && (tickets.length > 0 || staff.length > 0)) {
                    console.log("Cloud database key not found (404). Initializing online database with local records...");
                    syncOnline();
                }
                if (callback) callback();
            });
